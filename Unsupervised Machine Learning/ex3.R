...
cca <- "
    ...
    for (m in 1:M) {
      ...
      for (d in 1:Dm[m]) {
        ind = ind+1;
        W[,ind] ~ normal(0,t_alpha[m,]);// write the distribution
        X[,ind] ~ normal(multiply(Z,W[,ind]),1/sqrt(tau[m]));// write the distribution
      }
	  }
  }"
